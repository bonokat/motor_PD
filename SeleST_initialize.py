...
class Experiment():
    def __init__(self,_thisDir):        
        ...
        self.taskInfo = {
            'Experiment name': 'ari', # name of experiment (included in filename)
            'Participant ID': 000, # ID of participant (included in filename)
            ...
            'Change general settings?': True} # option to change general settings via GUI
        ...
        self.genSettings = {
            ...
            'n practice go trials': 20, # number of go trials to include in practice go-only block
            'n go trials per block': 24,
            'n stop-both trials per block': 8,
        ...
        self.advSettings = {
            ...
            'Right response key': '2', # response key for right stimulus
...
#        self.trigger_log = None
        if self.advSettings['Send serial trigger at trial onset?'] == True:
            ...
            # -------------------------------
            trigger_log_name = ("test_triggers_log.txt")
            trigger_log_path = os.path.join(_thisDir, trigger_log_name)

            self.trigger_log = open(trigger_log_path, "w")
            self.trigger_log.write("time_s\ttrial\ttrialType\ttrigger\n")
        ...
        self.instr_3_stop = visual.ImageStim(self.win, image=instrDir+'stop_practice_uni.png')
...
class Stimuli:
    def __init__(self, exp):
        ...
        self.photoD = visual.Rect(exp.win, fillColor='black', lineColor=None, opacity=0, units='pix', size=[100,100], pos=[-exp.win.size[0]/2, exp.win.size[1]/2]) # for triggering photodiode  , units='pix',
        # Create cues
