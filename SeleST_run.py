...
def Block(exp,trialInfo):
    ...
    if exp.advSettings['Send serial trigger at trial onset?'] == True: # serial trigger for block
...
#        exp.ser.write(f"WRITE {trigger_code} 5000 0\n".encode("utf-8"))
        ...
        exp.trigger_log.write(
            f"{timestamp:.6f}\t'nan'\t'nan'\t{trigger_code}\n"
        )
...
#        self.stopSignal = False # flag whether to present stop signal
...
class Start_Trial:
    def __init__(self,exp,stimuli,trialInfo,thisTrial,trial):
        ...
        self.stimList = [stimuli.L_stim, stimuli.R_stim, stimuli.L_stim2, stimuli.R_stim2, stimuli.photoD] # set list of stimuli to observe during a trial
...
#            exp.ser.write(f"WRITE {trigger_code} 5000 0\n".encode("utf-8"))
        ...
        pd_trig(exp, trialStimuli) # trigger photodiode
...
def endTask(exp, stimuli, trialStimuli):
    ...
    if exp.genSettings['Use response box?'] == True:
        ...
        #exp.rb.close()   
        pass       
    if hasattr(exp, "trigger_log"):
        exp.trigger_log.close()
    exp.win.close()
    core.quit()

# convenience function for triggering photodiode stimulus
def pd_trig(exp, trialStimuli):    
    trialStimuli.stimList[4].fillColor='black'
    for i in (1, 0):        
        trialStimuli.stimList[4].opacity=i #photodiode trigger for start of trial 
        trialStimuli.stimList[4].setAutoDraw(True)
